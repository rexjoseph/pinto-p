...
library LibFlood {
     ...
     */
    function startRain(uint256 caseId) internal {
        AppStorage storage s = LibAppStorage.diamondStorage();
    ...
    function floodPodline() private {
        ...
        uint256 totalBeanSupply = BeanstalkERC20(s.sys.bean).totalSupply();
        ...
        s.sys.fields[s.sys.activeField].harvestable = s
            ...
            .harvestable
            .add(sopFieldBeans);
        BeanstalkERC20(s.sys.bean).mint(address(this), sopFieldBeans);
    ...
    function sopWell(WellDeltaB memory wellDeltaB) private {
        ...
        if (wellDeltaB.deltaB > 0) {
            ...
            BeanstalkERC20(s.sys.bean).mint(address(this), sopBeans);

            // Approve and Swap Beans for the non-bean token of the SOP well.
            BeanstalkERC20(s.sys.bean).approve(wellDeltaB.well, sopBeans);
