...
import {ERC20Upgradeable, ERC20PermitUpgradeable} from "@openzeppelin-upgradeable/token/ERC20/extensions/ERC20PermitUpgradeable.sol";
import {IERC20, SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {IWell, Call} from "./interfaces/basin/IWell.sol";
import {IWellErrors} from "./interfaces/basin/IWellErrors.sol";
import {IPump} from "./interfaces/basin/pumps/IPump.sol";
import {IWellFunction} from "./interfaces/basin/IWellFunction.sol";
import {LibBytes} from "./libraries/LibBytes.sol";
import {ClonePlus} from "./utils/ClonePlus.sol";
...
contract Well is ERC20PermitUpgradeable, IWell, IWellErrors, ReentrancyGuardUpgradeable, ClonePlus {
    ...
    function _updatePumps(uint256 _numberOfTokens) internal returns (uint256[] memory reserves) {
        ...
        if (_numberOfPumps == 1) {
            ...
            for (uint256 i; i < _pumps.length; ++i) {
                // Don't revert if the update call fails.
                try IPump(_pumps[i].target).update(reserves, _pumps[i].data) {} catch {
                    // ignore reversion. If an external shutoff mechanism is added to a Pump, it could be called here.
                }
            }
        }
