...
abstract contract Invariable {
    ...
    modifier noSupplyChange() {
        ...
        uint256 initialSupply = BeanstalkERC20(LibAppStorage.diamondStorage().sys.bean)
            .totalSupply();
        _;
        require(
            BeanstalkERC20(LibAppStorage.diamondStorage().sys.bean).totalSupply() == initialSupply,
            "INV: Supply changed"
        );
    }
    ...
    function getTokenEntitlementsAndBalances(
        ...
        for (uint256 i; i < tokens.length; i++) {
            ...
            if (tokens[i] == s.sys.bean) {
                entitlements[i] += s.sys.orderLockedBeans;
                for (uint256 j; j < s.sys.fieldCount; j++) {
                    // Unharvested harvestable beans.
                    entitlements[i] += (s.sys.fields[j].harvestable - s.sys.fields[j].harvested);
                }
            }
