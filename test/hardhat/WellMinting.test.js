...
describe("Well Minting", function () {
  ...
  it("Initializes the Well Oracle", async function () {
    await beanstalk.wellOracleSnapshot(this.well.address);
  });
  ...
  describe("Delta B > 0", async function () {
    ...
    it("Checks a delta B > 0", async function () {
      expect(await beanstalk.poolDeltaB(this.well.address)).to.be.equal("147612846090");
    });
  });
  ...
  describe("Delta B < 0", async function () {
    ...
    it("Checks a delta B < 0", async function () {
      expect(await beanstalk.poolDeltaB(this.well.address)).to.be.equal("-203935707858");
    });
  });
