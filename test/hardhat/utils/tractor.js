...
const { BEAN, ZERO_ADDRESS, BEANSTALK, BEAN_ETH_WELL } = require("./constants.js");
const { to6 } = require("./helpers.js");
...
/**
 * Blueprint allowing the Operator to mow on behalf of the Publisher.
 * Operator is rewarded Beans as a ratio of stalk increase.
 */
...
/**
 * Blueprint allowing the Operator to Convert BEAN:ETH LP and Bean bidirectionally.
 * Operator is rewarded flat rate tip.
 */
const draftConvert = async (tip, minLpPerBeanRatio, minBeanPerLpRatio, verbose = false) => {
  // AdvancedFarmCall[]
  let advancedFarmCalls = [];
    ...
    callData: await wrapExternalCall(
      ...
      (await junctionInterface()).encodeFunctionData("bytes32Switch", [
        ...
        [
          ...
          ethers.utils.hexZeroPad(BEAN, 32), // left pad address
          ethers.utils.hexZeroPad(BEAN_ETH_WELL, 32), // left pad address
          ethers.utils.hexZeroPad(0, 32)
        ]
      ])
    ),
  ...
  if (verbose) {
    ...
    console.log(BEAN);
    console.log(ethers.utils.hexZeroPad(BEAN, 32));
    console.log(minLpPerBeanRatio);
    console.log(ethers.utils.hexZeroPad(minLpPerBeanRatio, 32));
  }
    ...
    callData: await wrapExternalCall(
      ...
      (await junctionInterface()).encodeFunctionData("bytes32Switch", [
        ...
        [
          ...
          ethers.utils.hexZeroPad(minLpPerBeanRatio, 32),
          ethers.utils.hexZeroPad(minBeanPerLpRatio, 32),
          ethers.constants.MaxUint256
        ]
      ])
    ),
  ...
  //     32 bytes - length of convertData (128) (shape depends on convert kind)
  //     32 bytes - convert kind (BEANS_TO_WELL_LP)
  //     32 bytes - amountIn
  ...
  advancedFarmCalls.push({
    callData: convertFacetInterface.encodeFunctionData("convert", [
      ethers.utils.defaultAbiCoder.encode(
        ["uint256", "uint256", "uint256", "address"], // type, amountIn, minAmountOut, nonBeanToken
        [TO_FILL, TO_FILL, TO_FILL, BEAN_ETH_WELL]
      ), // convertData
      [TO_FILL], // stems
      [TO_FILL] // amounts
    ]),
    clipboard: await drafter().then(
      async (drafter) =>
        await drafter.encodeClipboard(0, [
          ...
          await drafter.encodeLibReturnPasteParam(3, SLOT_SIZE, ARGS_START_INDEX + SLOT_SIZE * 11)
        ])
    )
  });
  // convert type.
  ...
  operatorPasteInstrs.push(
    await drafter().then(
      async (drafter) =>
        await drafter.encodeOperatorPasteInstr(SLOT_SIZE, 5, ARGS_START_INDEX + SLOT_SIZE * 9)
    )
  );
