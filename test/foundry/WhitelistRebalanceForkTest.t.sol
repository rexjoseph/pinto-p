...
contract WhitelistRebalanceForkTest is TestHelper {
    ...
    function test_forkBase_whitelistRebalance() public {
        ...
        forkMainnetAndUpgradeAllFacets(forkBlock, vm.envString("BASE_RPC"), PINTO, "InitWhitelistRebalance");
        ...
        assertEq(bs.tokenSettings(PINTO_CBETH_LP).optimalPercentDepositedBdv, EXPECTED_ALLOCATION, "CBETH LP allocation incorrect");
        assertEq(bs.tokenSettings(PINTO_CBBTC_LP).optimalPercentDepositedBdv, EXPECTED_ALLOCATION, "CBBTC LP allocation incorrect");
        assertEq(bs.tokenSettings(PINTO_USDC_LP).optimalPercentDepositedBdv, EXPECTED_ALLOCATION, "USDC LP allocation incorrect");
        ...
        for (uint256 i = 0; i < whitelistedTokens.length; i++) {
            ...
            if (i > 0) {
                totalAllocation += uint256(bs.tokenSettings(whitelistedTokens[i]).optimalPercentDepositedBdv);
            }
        }
