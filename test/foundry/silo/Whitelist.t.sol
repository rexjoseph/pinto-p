...
import {GaugeFacet, IGaugeFacet} from "contracts/beanstalk/facets/sun/GaugeFacet.sol";
...
contract WhitelistTest is TestHelper {
    ...
    function test_whitelistRevertInvalidlwImplementation(uint i) public prank(BEANSTALK) {
        bytes4 gpSelector = IMockFBeanstalk.defaultGaugePoints.selector;
    ...
    function verifyWhitelistEvents(
        ...
        vm.expectEmit();
        emit AddWhitelistStatus(token, 3, true, true, true, true);
        vm.expectEmit();
