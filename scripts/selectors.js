...
async function printSelectors(contractName, artifactFolderPath = "../out") {
  ...
  return [
    contractName,
    contractArtifact.methodIdentifiers,
    Object.values(contractArtifact.methodIdentifiers).map((element) => "0x" + element)
  ];
}
...
async function printAllSelectors(
  contractNames,
  defaultArtifactFolderPath = "../out",
  verbose = true
) {
  const promises = contractNames.map((contractName) =>
