...
async function upgradeWithDeployedFacets({
  ...
  if (verbose) {
    console.log("------");
    console.log("initFacetCallGas: " + strDisplay(initFacetCallGas));
  }
  if (verbose) {
    console.log("------");
    console.log("Upgrade transaction hash: " + result.hash);
...
async function upgradeWithNewFacets({
  ...
  for (const [name, facetFactory] of undeployed) {
    ...
    const replace = [];
    const selectors =
      selectorsToAdd[name] !== undefined ? selectorsToAdd[name] : getSelectors(deployedFactory);
    for (const selector of selectors) {
      if (!inFacets(selector, existingFacets)) {
        add.push(selector);
      } else {
    ...
    if (replace.length > 0) {
      diamondCut.push([deployedFactory.address, FacetCutAction.Replace, replace]);
    }
  }
  if (verbose) {
    console.log("diamondCut arg:");
